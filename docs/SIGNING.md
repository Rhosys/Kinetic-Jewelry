...
Generated once with `generate-android-keystore --alias lyra` from the `_tools` repo — never needs regenerating unless the Play Store upload key is revoked.

## Generating the keystore

```bash
generate-android-keystore --alias lyra > deployment/android-upload-signing.json
```
...
- **keystore** — RSA 4096, PKCS12, 30-year validity, alias `lyra`. Password-protected; safe to commit.
...
Key alias is `lyra`. Store password and key password are the same value (PKCS12 uses one password). Plaintext exists only for the duration of the CI job.
...
No other CI/CD secrets are required. The OIDC trust policy on the IAM role restricts access to the `rhosys/rapid/kinetic-jewelry` project path.
...
1. Generate a new keystore: `generate-android-keystore --alias lyra > deployment/android-upload-signing.json`
2. Export the new upload certificate: `keytool -export -alias lyra -keystore <decoded-keystore> -rfc > upload_cert.pem`
