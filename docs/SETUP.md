...
Local development environment setup for the Lyra Android app (`ch.rhosys.lyra`).
...
This creates an AVD named `LyraAVD` using the Pixel 7 device profile with `system-images;android-35;google_apis;x86_64`. If the AVD already exists, the script exits cleanly without recreating it.
...
| `scripts/emulator-create.sh` | Create the LyraAVD emulator |
| `scripts/emulator-start.sh` | Start the LyraAVD emulator (foreground) |
| `scripts/emulator-delete.sh` | Delete the LyraAVD emulator |
...
The debug build uses application ID `ch.rhosys.lyra.debug` so it can be installed alongside the release version.
...
If the variable is still missing, check that the marker block `# BEGIN lyra android sdk` exists in your profile file.
